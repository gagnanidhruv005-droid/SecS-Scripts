...
# ── Detect Ubuntu / Suricata version to avoid suricata-update conflict ────────
# On Ubuntu 24.04 with Suricata 8.x, suricata-update is BUNDLED inside the
# main suricata package. Installing it separately causes a dpkg file-conflict:
#   "trying to overwrite /usr/bin/suricata-update, which is also in package suricata"
# Fix: install suricata first, then only install suricata-update standalone
# if it is NOT already provided by the suricata package.

info "Installing Suricata..."
apt-get install -y suricata jq curl dnsutils

# Repair any broken dpkg state left from a previous failed install attempt
info "Ensuring dpkg is in a clean state..."
dpkg --configure -a
apt-get install -f -y

# Check if suricata-update is already bundled (Suricata 8.x on Ubuntu 24.04)
if command -v suricata-update &>/dev/null; then
    info "suricata-update is already bundled with this Suricata version — skipping separate install."
else
    info "suricata-update not found — installing standalone package..."
    # Use --no-install-recommends to avoid pulling in conflicting files
    apt-get install -y --no-install-recommends suricata-update \
      || {
        warn "Standalone suricata-update install failed. Trying pip fallback..."
        pip3 install suricata-update --break-system-packages \
          && info "suricata-update installed via pip." \
          || error "Could not install suricata-update by any method. Aborting."
      }
fi

# Install python3-pip separately (not bundled, safe on all versions)
apt-get install -y python3-pip || true

info "Verifying installation..."
suricata --version
suricata-update --version
info "Suricata installed. Capture interface: $IFACE"
...
suricata-update enable-source abuse.ch/urlhaus 2>/dev/null || true   # Malicious URLs
suricata-update enable-source abuse.ch/sslbl   2>/dev/null || true   # TLS cert blocklist
